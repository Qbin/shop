...
try:
    ...
    _DEFAULTLOCALE = ('en_US', 'UTF-8')[0].split("_")[0]
...
def get_holidays(dt):
    """ dt is solar datetime
    """
    result = []
    try:
        (lun_year, lun_month, lun_day, is_leap) = sol2lun(dt.year, dt.month, dt.day)
    except:
        return []
    holiday = lunar_holiday(lun_month, lun_day)
    if holiday:
        result.append(holiday)
    
    holiday = solar_holiday(dt.month, dt.day)
    if holiday:
        result.append(holiday)
    return result
...
def lunar_holiday(month, day):
    """ year, month is lunar date
    """
    key = "%02d%02d" % (month, day)
